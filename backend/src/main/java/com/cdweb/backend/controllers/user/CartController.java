...
public class CartController {
    ...
    @PostMapping("/update")
    public ResponseEntity<?> updateCart(@RequestBody CartItemRequest request){
        CartItemResponse response = cartService.updateCart(request);
        return ResponseEntity.status(HttpStatus.OK).body(
                (response != null)
    ...
    public ResponseEntity<?> getListCart(HttpServletRequest httpRequest){
        ...
        String token = authorizationHeader.substring(Constant.BEARER.length());
        Users user =  jwtService.getUserFromToken(token);
        List<CartItemResponse> responses = cartService.findByUserOrderByCreatedDateDesc(user);
        return ResponseEntity.status(HttpStatus.OK).body(
                (responses.size()> 0)
